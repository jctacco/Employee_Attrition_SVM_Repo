...
if __name__ == "__main__":
    # Load the dataset
    data_source = "Employee_Attrition_SVM_Repo\\data\\raw\\HR_Employee_Attrition.csv"
    ...
    plot_histograms(df, num_cols, "Employee_Attrition_SVM_Repo\\reports\\figures\\histograms.png")
    
    view_percentage_of_subcategories(df, cat_cols)    
    
    plot_barcharts(df, cat_cols, "Employee_Attrition_SVM_Repo\\reports\\figures\\barcharts.png")
    
    view_mean_of_each_numerical_variable(df, num_cols)
    
    print("Let's check relationship of numerical variables:")
    plot_correlation_heatmap(df, num_cols, "Employee_Attrition_SVM_Repo\\reports\\figures\\correlation_matrix_heatmap.png")
    ...
    with open('Employee_Attrition_SVM_Repo\\models\\logistic_regression.pkl', 'wb') as f:
    ...
    metrics_score(y_train, y_pred_train_lr, "Employee_Attrition_SVM_Repo\\reports\\figures\\LR_Model_trainset_metrics_report.png")
    
    #checking the performance on the test dataset
    y_pred_test_lr = make_predictions(lrmodel, X_test)
    metrics_score(y_test, y_pred_test_lr, "Employee_Attrition_SVM_Repo\\reports\\figures\\LR_Model_testset_metrics_report.png")
    ...
    # Save the trained model
    with open('Employee_Attrition_SVM_Repo\\models\\SVM.pkl', 'wb') as f:
        pickle.dump(SVM, f)

    #checking the performance on the training data
    y_pred_train_SVM = make_predictions(lrmodel,X_train)
    metrics_score(y_train, y_pred_train_SVM, "Employee_Attrition_SVM_Repo\\reports\\figures\\SVM_trainset_metrics_report.png")
    
    #checking the performance on the test dataset
    y_pred_test_SVM = make_predictions(lrmodel, X_test)
    metrics_score(y_test, y_pred_test_SVM, "Employee_Attrition_SVM_Repo\\reports\\figures\\SVM_testset_metrics_report.png")
